...
when a user selects a result, show pop up, that contains :

count / servings / fluid ounces 

the user can choose to fill only one of the following. if others arent filled that is fine. 

if user enters strawberry and click son teh result this pop up comes up and user chooses count = 4 then that means the calories increased x 4.

so that means the ai that is providing the results for ingredeints is approximating one item 

as for a meal, its full calorie count, unless user puts (here comes serving:) 2 servings which doubles the calories.

and if user inputs a drink and changes the fluid ounces then the drinks calories ioncrease aswell.

now heres the tricky bit. i want ai to detect wether ingredient / meal / drink 

and the pop up will show count if ingredeint 
pop up will show serving if meal 
pop up will show fluid ounce if drink

understood? 

have the ai identify wether ingredeint / meal/ drink when user wnats a result before pop up, so the pop immeditley comes up after. 

this value of count/ serving / fluid should be saved. because on its respective meal card we will show 
