                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              ...
                              style: TextStyle(
                                color:
                                    risk.isUnsafe ? Colors.red : Colors.green,
                        ...
                        Builder(
                          builder: (_) {
                            ...
                            final items = [
                              ...allergens.map(
                                (a) =>
                                    '${AppLocalizations.of(context)!.contains} $a',
                              ),
                              if (traces.isNotEmpty)
                                '${AppLocalizations.of(context)!.contains} $traces',
                            ];
                            if (items.isEmpty) {
                              return Container(
...
class _HealthScoreBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isUnknown =
        grade.isEmpty || grade == 'unknown' || grade.contains('UNKNOWN');
    Color badgeColor;
...
_RiskAssessment _deriveRisk(Product product) {
  ...
  if (product.nutriScoreGrade.isNotEmpty &&
      ['d', 'e'].contains(product.nutriScoreGrade.toLowerCase())) {
  ...
  if (product.ingredients.any(
    (i) =>
        i.toLowerCase().contains('milk') ||
        i.toLowerCase().contains('egg') ||
        i.toLowerCase().contains('meat'),
  )) {
...
Map<String, Object?> _inferFoodTypeAndQuantity(Product p) {
  ...
  bool isDrink =
      cats.any(
        (c) =>
            c.contains('beverages') ||
            c.contains('drinks') ||
            c.contains('beverage'),
      ) ||
      serving.contains('ml') ||
      serving.contains('fl oz');
  bool isIngredient = cats.any(
    (c) =>
        c.contains('ingredients') ||
        c.contains('spices') ||
        c.contains('condiments'),
  );
  final String foodType =
      isDrink ? 'drink' : (isIngredient ? 'ingredient' : 'meal');
  if (foodType == 'drink') {
