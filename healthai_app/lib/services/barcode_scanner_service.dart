...
class BarcodeScannerService {
  ...
  Future<String?> processCameraImage(
    ...
    try {
      ...
      for (final format in formats) {
        try {
          for (final r in <InputImageRotation>{
            rotation,
            ...
            if (bytes.length <
                (imageSize.width * imageSize.height * 1.5).toInt()) {
  ...
  Future<BarcodeGuidance?> processCameraImageForGuidance(
    CameraImage image, {
    required InputImageRotation rotation,
  }) async {
    if (_isClosed) return null;
