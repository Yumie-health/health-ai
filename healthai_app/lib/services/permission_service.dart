...
class PermissionService {
  ...
  requestAllPermissions() async {
    ...
    if (!isAndroid13Plus) {
      ...
      print(
        '🖼️ Skipping photos permission on Android 13+ (uses photo picker)',
      );
    }
    ...
    results[ph.Permission.notification] =
  ...
  static Future<bool> shouldRequestPermissions() async {
    ...
    final notificationsGranted = await isPermissionGranted(
      ph.Permission.notification,
    );

    // Only check photos permission on iOS or Android 12 and below
    bool photosGranted =
        true; // Default to true for Android 13+ (no permission needed)
    if (!isAndroid13Plus) {
  ...
  static Future<ph.PermissionStatus> requestPermission(
  ...
  static Future<Map<ph.Permission, ph.PermissionStatus>>
  requestPermissionsWithExplanation(BuildContext context) async {
    final results = <ph.Permission, ph.PermissionStatus>{};
    final isAndroid13Plus = await _isAndroid13OrHigher();
    ...
    if (!await isPermissionGranted(ph.Permission.camera)) {
      ...
      if (shouldRequest) {
        results[ph.Permission.camera] = await requestPermission(
    ...
    if (!isAndroid13Plus) {
      ...
      if (shouldRequestPhotos) {
        results[ph.Permission.photos] = await requestPermission(
    ...
    if (!await isPermissionGranted(ph.Permission.notification)) {
      ...
      if (shouldRequest) {
        ...
        results[ph.Permission.notification] = await requestPermission(
          ph.Permission.notification,
        );
      } else {
  ...
  static String getPermissionStatusText(
